...
void uartHandler(void *pvNotUsed) {
	...
	configASSERT(&uartDevice == pvNotUsed);
	...
	ulActiveInterrupts = XUartPs_ReadReg(UART_DEVICE_BASEADDR,  XUARTPS_IMR_OFFSET); // Read the interrupt ID register to see which interrupt is active.
	ulActiveInterrupts &= XUartPs_ReadReg(UART_DEVICE_BASEADDR,  XUARTPS_ISR_OFFSET);
	...
	if((ulActiveInterrupts & serRECEIVE_INTERRUPT_MASK) != 0) { // Are any receive events of interest active?
		...
		ulChannelStatusRegister = XUartPs_ReadReg(UART_DEVICE_BASEADDR, XUARTPS_SR_OFFSET); // Read the Channel Status Register to determine if there is any data in the RX FIFO.

		xil_printf("From cam: ");
		while((ulChannelStatusRegister & XUARTPS_SR_RXEMPTY) == 0) { // Move data from the Rx FIFO to the Rx queue.
			char receiveChar;
			receiveChar = XUartPs_ReadReg(UART_DEVICE_BASEADDR, XUARTPS_FIFO_OFFSET);
			xil_printf("%c", receiveChar);

			xQueueSendFromISR(rxQueue, &receiveChar, &xHigherPriorityTaskWoken);
			ulChannelStatusRegister = XUartPs_ReadReg(UART_DEVICE_BASEADDR, XUARTPS_SR_OFFSET);
		}
		xil_printf("\n\r");
	}
	...
	if((ulActiveInterrupts & serTRANSMIT_IINTERRUPT_MASK) != 0) // Are any transmit events of interest active?
	{
		if( uartDevice.SendBuffer.RemainingBytes == 0 )
		{
			...
			XUartPs_WriteReg( UART_DEVICE_BASEADDR, XUARTPS_IDR_OFFSET, XUARTPS_IXR_TXEMPTY ); // No more data to transmit.
		}
		else
		{
	...
	XUartPs_WriteReg(UART_DEVICE_BASEADDR, XUARTPS_ISR_OFFSET, ulActiveInterrupts); // Clear the interrupt status.
